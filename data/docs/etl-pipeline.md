...
title: "ETL Pipeline"
category: "ETL"
source: "ETL Pipeline __ Spring AI Reference.pdf"
generated_by: "springai-kb-converter"
---

# ETL Pipeline

The Extract, Transform, Load (ETL) framework orchestrates data processing for Retrieval Augmented Generation (RAG). It guides data from raw sources to a structured vector store optimized for retrieval by AI models.

## Overview

ETL pipelines create, transform, and store `Document` instances. The `Document` class holds text, metadata, and optionally media (images, audio, video).

Three main ETL components:

- `DocumentReader` — implements `Supplier<List<Document>>`
- `DocumentTransformer` — implements `Function<List<Document>, List<Document>>`
- `DocumentWriter` — implements `Consumer<List<Document>>`

**Basic pipeline example** (Java functional style):

```java
vectorStore.accept(tokenTextSplitter.apply(pdfReader.get()));
```

Or using domain-friendly method names:
...
The `JsonReader` processes JSON documents, converting them into `Document` objects.
...
- `resource` — Spring `Resource` object pointing to the JSON file.
- `jsonKeysToUse` — Array of JSON keys to use as text content in resulting `Document` objects.
- `jsonMetadataGenerator` — Optional `JsonMetadataGenerator` to create metadata for each document.

**Behavior:**

- Handles both JSON arrays and single JSON objects.
- Extracts content based on specified `jsonKeysToUse`.
- If no keys specified, uses the entire JSON object as content.
- Generates metadata using provided `JsonMetadataGenerator` (or empty if not provided).

**Using JSON Pointers:**

Retrieve specific parts of a JSON document using JSON Pointers (RFC 6901):
...
- Handles large JSON files efficiently via streaming for arrays.
- Multiple `jsonKeysToUse` concatenates values.

### Text

The `TextReader` processes plain text documents into `Document` objects.
...
- `setCharset(Charset charset)` — Sets character encoding (default: UTF-8).
- `getCustomMetadata()` — Returns mutable map for adding custom metadata.

**Behavior:**

- Reads entire file into a single `Document`.
- Auto-adds metadata: `charset`, `source`, custom metadata.

**Notes:**

- Reads entire file into memory (not suitable for very large files).
- To split text into chunks, use `TokenTextSplitter` after reading.
- Uses Spring's `Resource` abstraction (classpath, filesystem, URLs).

### HTML (JSoup)

The `JsoupDocumentReader` processes HTML documents using JSoup.
...
- `separator` — String joining text from multiple selected elements (default: "\n").
- `allElements` — Extract all text from `<body>` (default: false).
- `groupByElement` — Create separate `Document` per selector match (default: false).
- `includeLinkUrls` — Extract and add absolute link URLs to metadata (default: false).
- `metadataTags` — List of `<meta>` tag names to extract (default: `["description", "keywords"]`).
- `additionalMetadata` — Add custom metadata to all `Document` objects.

**Behavior:**

- `selector` determines which elements are used for text extraction.
- If `allElements` is true, all text within `<body>` is extracted into a single `Document`.
- If `groupByElement` is true, each element matching `selector` creates a separate `Document`.
- Document title, meta tag content, and (optionally) link URLs are added to metadata.
- HTML tags are removed; only text content is preserved.

### Markdown

The `MarkdownDocumentReader` processes Markdown documents.
...
- `horizontalRuleCreateDocument` — Horizontal rules create new `Document` objects (default: false).
- `includeCodeBlock` — Include code blocks with surrounding text or separate (default: false = separate).
- `includeBlockquote` — Include blockquotes with surrounding text or separate (default: false = separate).
- `additionalMetadata` — Add custom metadata to all created `Document` objects.

**Behavior:**

- Headers become metadata.
- Paragraphs become document content.
- Code blocks and blockquotes handled per configuration.
- Formatting like inline code, lists, and styling preserved.

### PDF Page

The `PagePdfDocumentReader` uses Apache PdfBox to parse PDFs.
...
The `ParagraphPdfDocumentReader` uses PDF catalog (TOC) information to split into paragraphs, outputting one `Document` per paragraph.

**Note:** Not all PDFs contain a PDF catalog.
...
The `TikaDocumentReader` uses Apache Tika to extract text from many document formats: PDF, DOC/DOCX, PPT/PPTX, HTML, etc.

See [Tika documentation](https://tika.apache.org/) for comprehensive format support.
...
Abstract base class for splitting documents to fit AI model context windows.

### TokenTextSplitter

Splits text into chunks based on token count using CL100K_BASE encoding.
...
1. `TokenTextSplitter()` — Default settings.
2. `TokenTextSplitter(boolean keepSeparator)` — Custom separator behavior.
3. `TokenTextSplitter(int chunkSize, int minChunkSizeChars, int minChunkLengthToEmbed, int maxNumChunks, boolean keepSeparator, List<Character> punctuationMarks)` — Full customization.

**Parameters:**

- `chunkSize` — Target chunk size in tokens (default: 800).
- `minChunkSizeChars` — Minimum chunk size in characters (default: 350).
- `minChunkLengthToEmbed` — Minimum length to include a chunk (default: 5).
- `maxNumChunks` — Maximum chunks to generate (default: 10000).
- `keepSeparator` — Retain separators like newlines (default: true).
- `punctuationMarks` — Characters for sentence boundaries (default: `.` `?` `!` `\n`).

**Behavior:**

1. Encodes input into tokens using CL100K_BASE.
2. Splits encoded text into chunks by `chunkSize`.
3. For each chunk:
   - Decodes back to text.
   - Finds suitable break points at `punctuationMarks` after `minChunkSizeChars`.
   - Trims and optionally removes newlines per `keepSeparator`.
   - Adds to output if longer than `minChunkLengthToEmbed`.
4. Continues until all tokens processed or `maxNumChunks` reached.
5. Remaining text added as final chunk if appropriate.
...
- Uses CL100K_BASE encoding from `jtokkit` (compatible with newer OpenAI models).
- Attempts semantic chunking by breaking at sentence boundaries.
- Metadata from original documents preserved and copied to chunks.
- Content formatter copied to derived chunks if `copyContentFormatter` is true (default).
- Particularly useful for preparing text for LLMs with token limits.
- Default punctuation marks work well for English; customize for other languages.
- Small texts (token count ≤ chunk size) not split at punctuation markers (prevents unnecessary fragmentation).
...
Uses a generative AI model to extract keywords from document content and add to metadata.
...
1. `KeywordMetadataEnricher(ChatModel chatModel, int keywordCount)` — Default template with specified keyword count.
2. `KeywordMetadataEnricher(ChatModel chatModel, PromptTemplate keywordsTemplate)` — Custom template.

**Behavior:**

1. For each input document, creates a prompt using the document's content.
2. Sends prompt to `ChatModel` to generate keywords.
3. Adds keywords to document metadata under "excerpt_keywords".
...
- Requires functioning `ChatModel`.
- Keyword count must be ≥ 1.
- Keywords returned as comma-separated string.
- Useful for improving searchability and creating tags/categories.
- If `keywordsTemplate` is set in builder, `keywordCount` is ignored.

### SummaryMetadataEnricher

Uses a generative AI model to create summaries for documents and adjacent documents.
...
- `chatModel` — AI model for generating summaries.
- `summaryTypes` — List of `SummaryType` enum values (PREVIOUS, CURRENT, NEXT).
- `summaryTemplate` — Custom template (optional).
- `metadataMode` — How to handle document metadata (optional).

**Behavior:**

1. For each input document, creates a prompt using the document's content and summary template.
2. Sends prompt to `ChatModel` to generate summary.
3. Depending on `summaryTypes`, adds metadata to each document:
   - `section_summary` — Summary of current document.
   - `prev_section_summary` — Summary of previous document (if available).
   - `next_section_summary` — Summary of next document (if available).
4. Returns enriched documents.
...
```java
ChatModel chatModel = // initialize
...
for (Document doc : enrichedDocs) {
    ...
    System.out.println("Previous: " + doc.getMetadata().get("prev_section_summary"));
    System.out.println("Next: " + doc.getMetadata().get("next_section_summary"));
}
```

**Expected behavior (two-document list):**

- Both documents receive `section_summary`.
- First document receives `next_section_summary` but no `prev_section_summary`.
- Second document receives `prev_section_summary` but no `next_section_summary`.
- First document's `section_summary` matches second document's `prev_section_summary`.
- First document's `next_section_summary` matches second document's `section_summary`.

**Notes:**

- Requires functioning `ChatModel`.
- Handles document lists of any size, properly handling first/last documents.
- Useful for creating context-aware summaries.
- `MetadataMode` parameter controls how existing metadata is incorporated.
...
The `FileDocumentWriter` writes `Document` objects to a file.
...
- `withDocumentMarkers` — Include document markers (default: false).
- `metadataMode` — Specifies what document content to write (default: MetadataMode.NONE).
- `append` — Append to file if exists (default: false).

**Document markers format:**
...
- Uses `FileWriter` with system default character encoding.
- Throws `RuntimeException` on write errors.
- Useful for debugging and creating human-readable outputs.

### VectorStore

Provides integration with various vector stores. See Vector DB Documentation for full listing.
---
title: "ETL Pipeline"
category: "ETL"
source: "ETL Pipeline __ Spring AI Reference.pdf"
...
The ETL pipeline orchestrates the flow from raw data sources to a structured vector store, ensuring data is in the optimal format for retrieval
by the AI model.

The RAG use case is text to augment the capabilities of generative models by retrieving relevant information from a body of data to enhance
the quality and relevance of the generated output.
...
The ETL pipeline is composed of the following interfaces and implementations. Detailed ETL class diagram is shown in the ETL Class
Diagram section.
...
In this example, if the `JsonReader`  is configured with `"description"`  as the `jsonKeysToUse` , it will create `Document`  objects where
the content is the value of the "description" field for each bike in the array.
...
The reader is flexible and can be adapted to various JSON structures by customizing the `jsonKeysToUse`  and
...
If neither `allElements`  nor `groupByElement`  is `true` , text from all elements matching the `selector`  is joined using the
`separator` .
...
`includeCodeBlock` : When set to `true` , code blocks will be included in the same `Document`  as the surrounding text. When `false` ,
code blocks create separate `Document`  objects.
`includeBlockquote` : When set to `true` , blockquotes will be included in the same `Document`  as the surrounding text. When
`false` , blockquotes create separate `Document`  objects.
`additionalMetadata` : Allows you to add custom metadata to all created `Document`  objects.
...
The `ParagraphPdfDocumentReader`  uses the PDF catalog (e.g. TOC) information to split the input PDF into text paragraphs and output a
single `Document`  per paragraph. NOTE: Not all PDF documents contain the PDF catalog.
...
The `TikaDocumentReader`  uses Apache Tika to extract text from a variety of document formats, such as PDF, DOC/DOCX, PPT/PPTX,
and HTML. For a comprehensive list of supported formats, refer to the Tika documentation.
...
The `TokenTextSplitter`  is an implementation of `TextSplitter`  that splits text into chunks based on token count, using the
...
```
You can customize the punctuation marks used for splitting text into semantically meaningful chunks. This is particularly useful for
...
3. `TokenTextSplitter(int chunkSize, int minChunkSizeChars, int minChunkLengthToEmbed, int maxNumChunks,`
`boolean keepSeparator, List<Character> punctuationMarks)` : Full constructor with all customization options.
...
b. Only if the total token count exceeds the chunk size, it attempts to find a suitable break point (using the configured
`punctuationMarks` ) after the `minChunkSizeChars` .
...
```java
Document doc1 = new Document("This is a long piece of text that needs to be split into smaller chunks for
processing.",
        Map.of("source", "example.txt"));
...
The `TokenTextSplitter`  uses the CL100K_BASE encoding from the `jtokkit`  library, which is compatible with newer OpenAI
models.
...
The content formatter (if set) from the original document is also copied to the derived chunks if `copyContentFormatter`  is set to
`true`  (default behavior).
This splitter is particularly useful for preparing text for large language models that have token limits, ensuring that each chunk is within
...
Extensibility: The `getLastPunctuationIndex(String)`  method is `protected` , allowing subclasses to override the punctuation
detection logic for specialized use cases.
Small Text Handling: As of version 2.0, small texts (with token count at or below the chunk size) are no longer split at punctuation
marks, preventing unnecessary fragmentation of content that already fits within the size limits.
...
1. `KeywordMetadataEnricher(ChatModel chatModel, int keywordCount)` : To use the default template and extract a specified
number of keywords.
...
// Or use custom templates
KeywordMetadataEnricher enricher = KeywordMetadataEnricher.builder(chatModel)
                .keywordsTemplate(new PromptTemplate("Extract 5 important keywords from the following text and
separate them with commas:\n{context_str}"))
                .build();
```

```java
Document doc = new Document("This is a document about artificial intelligence and its applications in modern
technology.");
...
The `SummaryMetadataEnricher`  is a `DocumentTransformer`  that uses a generative AI model to create summaries for documents and
add them as metadata. It can generate summaries for the current document, as well as adjacent documents (previous and next).
...
1. `SummaryMetadataEnricher(ChatModel chatModel, List<SummaryType> summaryTypes)`
2. `SummaryMetadataEnricher(ChatModel chatModel, List<SummaryType> summaryTypes, String summaryTemplate,`
```text
...
This enricher is particularly useful for creating context-aware summaries, allowing for better understanding of document relationships in
a sequence.
...
1. `FileDocumentWriter(String fileName)`
2. `FileDocumentWriter(String fileName, boolean withDocumentMarkers)`
3. `FileDocumentWriter(String fileName, boolean withDocumentMarkers, MetadataMode metadataMode, boolean`
```text
append)
```
...
This will write all documents to "output.txt", including document markers, using all available metadata, and appending to the file if it already
exists.
